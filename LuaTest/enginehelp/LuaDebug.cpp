...
int ShowStack(lua_State* L)
{
	...
	while (lua_getstack(L, iIndex++, &debugInfo)) 
	{
		...
		{
			elex::strings::AppendF(&str,L"%s File(%d): %s\n",
				elex::strings::s2ws(debugInfo.short_src).c_str(),
				debugInfo.currentline,
				debugInfo.name ? elex::strings::s2ws(debugInfo.name).c_str() : L"");
		}
	}
...
void DebugInvokError(lua_State* L,HRESULT hr,const std::wstring& strScriptName,ILuaApi* pApi,DISP_ID dispid,const DISP_PARAMS& rParams)
{
	...
	if(pApi)
	{
		...
		for(auto it : table)
		{
			if(dispid == it.first)
			{
				strFuncName = elex::strings::s2ws(it.second);
				break;
			}
		}
	}
 	...
 	{	
		std::wstring str;
		elex::strings::AppendF(&str,_T("API函数传入参数错误脚本名称%s，函数名称: %s\r\n"),
			strScriptName.c_str(),
			strFuncName.c_str());
	...
	{
		std::wstring str;
		elex::strings::AppendF(&str,_T("API函数执行失败脚本名称%s，函数名称: %s\r\n错误码: %x\r\n"),
			strScriptName.c_str(),
			strFuncName.c_str(),
			hr);
		DumpArgsInfo(str, rParams);
...
int ErrorHandle(lua_State* L)
{
	...
	{			
		...
		_ASSERT_EXPR(0, elex::strings::s2ws(str).c_str());
	}
#endif
	return 1;
}
